...
def get_args() -> dict:
    ...
    if args.stdin:
        ...
        return {"stdin": False, "scope": scope, "nameprefix": nameprefix, "updated_at": updated_at, "verbose": args.verbose}


def checkTRSEcopy(m: dict) -> bool:
    """
    check Tape copy status. Currently just check the <RucioAttrTapeBkupKey> attribute in Rucio
    """
    
    if RucioAttrTapeBkupKey in m.keys() and \
        m[RucioAttrTapeBkupKey] == RucioAttrTapeBkupDone:
        return True
    else:
        return False
...
def checkFrDFcopy(m: dict, dids: dict) -> bool:
    """
    deep check the dataset at FrDF, by query the adler32 of files in the dataset
    """
    ...
    """
    with rucioOprLock:
        dsreplicas = ReplicaClient.list_dataset_replicas(scope=m['scope'], name=m['name'], deep=False)
        for x in dsreplicas:
    ...
    if not hasCopy:
        if config["verbose"]:
            with msgLock:
                print(f'  \U0001F7E1 {FrDFRSE} does not have dataset {m["scope"]}:{m["name"]} (according to Rucio)') 
        else:
            return False

    if config["verbose"]:
        with msgLock:
            print(f'  \U0001F7E1 Deep check dataset {m["scope"]}:{m["name"]} at {FrDFRSE}')

    #if config["verbose"]:
    #    with msgLock:
    #        print(f'    \U0001F7E1 This dataset contains {len(dids)} logical files:')

    replicas = []
    with rucioOprLock:
        xdids = dids.copy()
        while len(xdids) > 0:
            xReplicas = ReplicaClient.list_replicas(dids=xdids[:500], schemes=['davs'], rse_expression=FrDFRSE)
            for x in xReplicas:
                replicas.append(x)
            xdids = xdids[500:]
    ...
    for x in replicas:
        if config["verbose"]:
            if FrDFRSE in x['rses'].keys():
                with msgLock:
                    print(f'      replica for {x["scope"]}:{x["name"]} exists. ', end="")
            else:
                with msgLock:
    ...
    if lfnset != pfnset:
        if config["verbose"]:
            with msgLock:
                print(f'content of dataset {m["scope"]}:{m["name"]} is not fully replicated to {FrDFRSE}')
        return False
    #else:
...
def checkUSDFcopy(m: dict, dids: dict) -> bool:
    """
    Deep check the USDF copy, by re-calculate etag of .zip file members and compare that with those in embargo
    """
    #global etagpartsizes, embargoS3
    USDFRSE="SLAC_RAW_DISK"

    # USDFRSE uses lfn2pfn_algorithem = 'identity'
    storageprefixmap = {'weka': '/sdf/data/rubin/rses/lsst/rawdisk',
                        'embargo': {"bucket": "rubin-summit"}}

    if config["verbose"]:
        with msgLock:
            print(f'  \U0001F7E1 Deep check dataset {m["scope"]}:{m["name"]} at {USDFRSE}')
    ...
    for did in dids:
        ...
        page_iterator = paginator.paginate(Bucket=storageprefixmap['embargo']['bucket'],
                                           Prefix=name[:-4],
                                           PaginationConfig={'PageSize': 1000})
        for page in page_iterator:
            if 'Contents' in page:
                for x in page['Contents']:
                    s3items[x["Key"]] = {"Size": x["Size"], "ETag": x["ETag"].replace('"','')}      
        # A short keys is the last component of LSSTCam/20251231/MC_O_20251231_000001/MC_O_20251103_000150_R21_S12.json
        s3itemshortkeys = set()  
        ...
        with zipfile.ZipFile(wekapfn, mode = 'r') as myzip:
             ...
             if len(s3itemshortkeys-set(zipitems)) != 0:
                 if config["verbose"]:
                     with msgLock:
                         print("      Please check missing/extra items in the .zip: ", end="")
                         print(s3itemshortkeys-set(zipitems))
                 return False

             for zipitem in zipitems:
                 key = name[:-4] + "/" + zipitem
                 if not key in s3items.keys():
                     continue
                 with myzip.open(zipitem) as f: 
                     ...
                     if etag == s3items[key]["ETag"]:
                         ...
                         if config["verbose"]:
                             with msgLock:
                                 print(f'      {zipitem} @ {scope}:{name} ETag mismatch \u274c. Skip the rest!') 
                         return False
    return True 
...
def checkAll(m: dict):
    ...
    dids = []
    with rucioOprLock:
        # Combine metadata from DIDCOLUMN and JSON
        # note: DIDclient.get_metadata_bulk() only returns metadata in DIDCOLUMN.         
        jsonMetadata = DIDclient.get_metadata(scope=m['scope'], name=m['name'], plugin='JSON')
        m = m | jsonMetadata
    ...
    # A raw Obs dataset should have at least two files: a *.zip and a *_dimensions.yaml 
    ...
    if any(".zip" in s['name'] for s in dids) and any("_dimensions.yaml" in s['name'] for s in dids):
        if config['verbose']:
    ...
    with rucioOprLock:
        DIDclient.set_metadata(scope=m['scope'],
                               name=m['name'],
...
def addTask2Queue(mdata: dict):
    ...
    for m in mdata:
        ...
        if not config['stdin'] and m['updated_at'].timestamp() < config['updated_at'].timestamp():
            continue
...
def checkX509proxy():
    ...
    try:
        with open(cert, "rb") as cert_file:
            ...
            dt = int(certificate.not_valid_after_utc.timestamp() - datetime.datetime.now().timestamp())
            if (dt < 300):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=nWorkers) as executor:
        try:
            ...
            if config['stdin']:
                ...
                names = DIDclient.list_dids(scope=config['scope'],
                                            filters=({"name":config['nameprefix'] + "*"}),
                                            long=False, did_type='dataset')
            dsvec = []
            for name in names:
                if config['stdin']:
