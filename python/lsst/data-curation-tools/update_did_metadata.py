...
def update_did(scope: InternalScope, name: str, adler32: str, md5: str, filesize):
    """
...
def load_metadata():
    ...
    file_data = {}
    for m in metadata:
        with open(f'{m}.305.txt', 'r') as f:
            file_data[m] = f.readlines()
    metadata_val = []
    for adler32, md5, filesize in zip(file_data['adler32'], file_data['md5'], file_data['filesize']):
        adler_scope, rucio_adler, butler_adler = adler32.strip().split(' ')
        md5_scope, rucio_md5, butler_md5 = md5.strip().split(' ')
        filesize_scope, rucio_filesize, butler_filesize = filesize.strip().split(' ')
...
#        dids = json.load(f)
    ...
    failed = []
    for file in dids:
        logging.info("Updating %s:%s with metadata: %s", file['scope'], file['name'], file)
        scope = InternalScope(scope=file['scope'])
        current_metadata = get_metadata(scope, file['name'])
        logging.debug("Current metadata for %s:%s: %s", file['scope'], file['name'], file)
        current_vals = {k: v for k, v in current_metadata.items() if k in ['adler32', 'md5', 'bytes']}
        if verify_metadata(current_vals, file['old']):
            try:
                ...
                logging.error("DID %s:%s update failed due to %s", file['scope'], file['name'], e)
                failed.append(file)
                continue
            updated_metadata = get_metadata(scope, file['name'])
            updated_vals = {k: v for k, v in updated_metadata.items() if k in ['adler32', 'md5', 'bytes']}
            if verify_metadata(updated_vals, file):
                ...
                print("Failed to update or dry run")
        else:
            if verify_metadata(current_vals, file):
                logging.info("DID %s:%s already updated, skipping", file['scope'], file['name'])
            else:
                logging.info("DID %s:%s metadata doesn't match provided old metadata, skipping", file['scope'], file['name'])
    logging.info("Number of DIDs to update: %s, Number of updated DIDs: %s, Number of failed updates: %s", to_update, updated, failed)
